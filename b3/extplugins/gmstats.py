...
12:KillKenny?^7 TEAM:FREE KILLS:0 DEATHS:0 ASSISTS:0 PING:0 AUTH:--- IP:bot
    ...
    def grab_player(self, sclient):
        ...
        if len(players)>0:
          try:
            found_player = [p for p in players if sclient.name in p.name]
            if found_player: return found_player[0]
          except: 
            found_player=[]
        else:
            self.debug('No player found matching %s' % sclient.name )
...
#                fin.close()
    ...
    def cmd_resetmoney(self, data, client, cmd=None):
        if data:
            ...
            self.info('getting money')
            actual_money = int(cursor.getValue('money'))
    ...
    def update_location(self, client, reward=None):
            ...
            if status:
                if status.lower() == 'on':
                    ...
                    if weapexactname == "NVG":
                        ...
                        client.message("^3......ACTUALLY, ^1YOU CAN! YOU GET ^3autobuy ^6NVG! ")
...
### new commands
    ...
    def announceStats(self):
        ...
        for client in self.console.clients.getList():
            moneyvar = client.var(self, 'money_earned').value
            if moneyvar is None:
                moneyvar = 0
